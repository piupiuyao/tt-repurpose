...
@click.option("--step", default="all", show_default=True,
              type=click.Choice(["all", "extract", "analyze", "rewrite", "images", "animate", "assemble"]),
...
def main(url: str, style: str, output: str, step: str):
    ...
    if step in ("all", "images"):
        from steps.generate_images import run as generate_images
        generate_images(output_dir, style)
